...
def getCroppedImage(sand_image, car_x, car_y, angle, crop_size=40): 
    ...
    final = [np.expand_dims(im, axis=0)]

    return torch.FloatTensor(final).squeeze(4).permute(1, 0, 2, 3)
...
class Car(Widget):
    ...
    def move(self, rotation):
        ...
        # self.x = self.x.clip(0,longueur)
        # self.y = self.y.clip(0,largeur)
        self.rotation = rotation
        self.angle = self.angle + self.rotation
        # with open("./logs/final8.txt", 'a') as f:
        #     sys.stdout = f
        #     print("in move car's positions are ", self.pos)
...
class Game(Widget):
    ...
    def eval_update(self, dt):
        ...
        if self.total_timesteps < self.max_timesteps:
            if self.done:
                if self.total_timesteps != 0:
                    with open("./logs/eval_final8.txt", 'a') as f:
                        sys.stdout = f
                        print("Total Steps: ", self.total_timesteps, "Episode: ",episode_num, "Reward: ", episode_reward,"Episode TimeSteps: ", self.episode_timesteps, "Sand Penalty: ", sand_penalty, "NonSand Reward: ", nonsand_reward, "Distance Reward: ", distance_reward, "Goal Reward: ", goal_reward)
            ...
            if sand[int(self.car.x),int(self.car.y)] > 0:
                sand_pointer += 1
                if sand_pointer >= _max_episode_steps/100:
                    nonsand_counter = 0
                ...
                sand_pointer = 0
                if nonsand_counter >= _max_episode_steps/100:
                    sand_pointer = 0
            ...
            if nonsand_counter >= _max_episode_steps/10:                
                reward += 10
                nonsand_reward += 10
    ...
    def update(self, dt):
        ...
        if self.total_timesteps < self.max_timesteps:
            ...
            # with open("./logs/final8.txt", 'a') as f:
            #     sys.stdout = f
            #     print("Car values before move are x: %s y: %s"%(self.car.x, self.car.y))
            #     print("action value is ", action)
            #     print("action being passed is ", action.tolist()[0])
            ...
            # with open("./logs/final8.txt", 'a') as f:
            #     sys.stdout = f
            #     print("Car values after move are x: %s y: %s"%(self.car.x, self.car.y))
