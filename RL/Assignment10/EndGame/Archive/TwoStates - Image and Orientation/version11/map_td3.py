...
def getCroppedImage(sand_image, car_x, car_y, angle, crop_size=40): 
    ...
    #padding for boundary scenarios
...
class Game(Widget):
    ...
    def eval_update(self, dt):
        ...
        if self.total_timesteps < self.max_timesteps:
            if self.done:
                if self.total_timesteps != 0:
                    with open("./logs/eval_final7.txt", 'a') as f:
                        sys.stdout = f
                        print("Total Steps: ", self.total_timesteps, "Episode: ",episode_num, "Reward: ", episode_reward,"Episode TimeSteps: ", self.episode_timesteps, "Sand Penalty: ", sand_penalty, "NonSand Reward: ", nonsand_reward, "Goal Reward: ", goal_reward)
            ...
            if sand_pointer >= _max_episode_steps/4:
                reward -= 1
                sand_penalty += 1
                self.done = True
