...
class Actor(nn.Module):
    def __init__(self, action_dim, orientation, max_action):
...
class Critic(nn.Module):
    def __init__(self, action_dim, orientation):
...
class TD3(object):
  ...
  def __init__(self, action_dim, orientation, max_action):
        ...
        self.actor = Actor(action_dim, orientation, max_action).to(device)
        self.actor_target = Actor(action_dim, orientation, max_action).to(device)
        self.actor_target.load_state_dict(self.actor.state_dict())
        #print('actor state_dict is ', self.actor.state_dict())
        self.actor_optimizer = torch.optim.Adam(self.actor.parameters(), lr=learning_rate)
        #print('actor parameters are ', self.actor.parameters())
        self.actor_loss = []

        self.critic = Critic(action_dim, orientation).to(device)
        self.critic_target = Critic(action_dim, orientation).to(device)
        self.critic_target.load_state_dict(self.critic.state_dict())
        #print('critic state_dict is ', self.critic.state_dict())
        self.critic_optimizer = torch.optim.Adam(self.critic.parameters(), lr=learning_rate)
        #print('critic parameters are ', self.critic.parameters())
        self.critic_loss = []
