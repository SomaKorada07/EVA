...
class ReplayBuffer(object):
  ...
  def __init__(self, max_size=30000):#1e6
    self.storage = []
  ...
  def sample(self, batch_size):
    ...
    for i in ind:
      ...
      # with open("./logs/eval_final6.txt", 'a') as f:
      #   sys.stdout = f
      #   print('in sample')
      #   print(np.array(batch_states))
      #   print(np.array(batch_orientations))
...
def getCroppedImage(sand_image, car_x, car_y, angle, crop_size=40): 
    ...
    # with open("./logs/final6.txt", 'a') as f:
    #     sys.stdout = f
...
class Game(Widget):
    ...
    start_timesteps = 12000 #1e4 # 3000 #Number of iterations/timesteps before which the model randomly chooses an action, and after which it starts to use the policy network
    #eval_freq = 5 # How often the evaluation step is performed (after how many timesteps)
    max_timesteps = 36000 #5e5 # 5000 #Total number of iterations/timesteps
    max_episodes = 60  # max num of episodes
    eval_episode = 1
    ...
    def eval_update(self, dt):
        ...
        #     print("steps are ", self.total_timesteps)
        ...
        if self.total_timesteps < self.max_timesteps:
            ...
            next_state, new_orientation, reward, self.done, sand_penalty, nonsand_reward, distance_reward, boundary_penalty, goal_reward, goal_penalty = self.step(action) #new_distance,
    ...
    def step(self, action):
        ...
        return next_state, new_orientation, reward, self.done, sand_penalty, nonsand_reward, distance_reward, boundary_penalty, goal_reward, goal_penalty #new_distance, 
    ...
    def update(self, dt):
        ...
        if self.total_timesteps < self.max_timesteps:
            ...
            #     print('shape of state is ', state.size())
                #print('type of orientation is ', orientation.size())
                #print('type of reward is ', reward.size())
                #print('orientation is ', orientation)
