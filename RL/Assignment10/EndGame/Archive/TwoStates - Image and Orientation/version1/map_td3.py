# Self Driving Car

# Importing the libraries
import numpy as np
from random import random, randint
#import matplotlib.pyplot as plt
import time
import os
import math

import logging
import sys

import torch
import torch.nn as nn
import torch.nn.functional as F

# Importing the Kivy packages
from kivy.app import App
from kivy.uix.widget import Widget
from kivy.uix.button import Button
from kivy.graphics import Color, Ellipse, Line
from kivy.config import Config
from kivy.properties import NumericProperty, ReferenceListProperty, ObjectProperty
from kivy.vector import Vector
from kivy.clock import Clock
from kivy.core.image import Image as CoreImage
from PIL import Image as PILImage
from PIL import ImageDraw
from kivy.graphics.texture import Texture

# Importing the Dqn object from our AI in ai.py
from ai_td3_working_latest import TD3

# Adding this line if we don't want the right click to put a red point
Config.set('input', 'mouse', 'mouse,multitouch_on_demand')
Config.set('graphics', 'resizable', False)
Config.set('graphics', 'width', '1429')
Config.set('graphics', 'height', '660')

# Implementing Experience Replay
class ReplayBuffer(object):

  def __init__(self, max_size=1e6):
    self.storage = []
    self.max_size = max_size
    self.ptr = 0


  def add(self, state, new_state, orientation, new_orientation, action, reward, done_bool):
    data = (state, new_state, orientation, new_orientation, action, reward, done_bool)
    if len(self.storage) == self.max_size:
        self.storage[int(self.ptr)] = data
        self.ptr = (self.ptr + 1) % self.max_size
    else:
        self.storage.append(data)

  
  def sample(self, batch_size):
    ind = np.random.randint(0, len(self.storage), size=batch_size)
    batch_states, batch_next_states, batch_orientations, batch_next_orientations, batch_actions, batch_rewards, batch_dones = [], [], [], [], [], [], []
    for i in ind:
      state, next_state, orientation, next_orientation, action, reward, done = self.storage[i]
      batch_states.append(np.array(state, copy=False))
      batch_next_states.append(np.array(next_state, copy=False))
      batch_orientations.append(np.array(orientation, copy=False))
      batch_next_orientations.append(np.array(next_orientation, copy=False))
      batch_actions.append(np.array(action, copy=False))
      batch_rewards.append(np.array(reward, copy=False))
      batch_dones.append(np.array(done, copy=False))
    return np.array(batch_states), np.array(batch_next_states), np.array(batch_orientations), np.array(batch_next_orientations), np.array(batch_actions), np.array(batch_rewards).reshape(-1, 1), np.array(batch_dones).reshape(-1, 1)

# Introducing last_x and last_y, used to keep the last point in memory when we draw the sand on the map
last_x = 0
last_y = 0
n_points = 0
length = 0

seed = 0
save_models = True # Boolean checker whether or not to save the pre-trained model
torch.manual_seed(seed)
np.random.seed(seed)
action_dim = 1
max_action = 7 # SOMA earlier it was 10

_max_episode_steps = 500
episode_num = 0

# Initializing the last distance
distance = 0
orientation = 0

# Getting our AI, which we call "brain", and that contains our neural network that represents our Q-function
brain = TD3(action_dim, orientation, max_action)
replay_buffer = ReplayBuffer()
scores = []
im = CoreImage("./images/MASK1.png")

#logging.basicConfig(filename='TD3.log',level=logging.INFO)
logging.basicConfig(filename='Soma.log',
                            filemode='w',
                            #format='%(asctime)s,%(msecs)d %(name)s %(levelname)s %(message)s',
                            #datefmt='%H:%M:%S',
                            level='DEBUG')

file_name = "%s_%s" % ("TD3", str(seed))
print ("---------------------------------------")
print ("Settings: %s" % (file_name))
print ("---------------------------------------")

if not os.path.exists("./results"):
    os.makedirs("./results")
if save_models and not os.path.exists("./pytorch_models"):
    os.makedirs("./pytorch_models")


# Initializing the map
first_update = True
def init():
    global sand
    global goal_x
    global goal_y
    global first_update
    global orientation
    sand = np.zeros((longueur,largeur))
    img = PILImage.open("./images/mask.png").convert('L')
    sand = np.asarray(img)/255
    goal_x = 1000
    goal_y = 400
    first_update = False
    global swap
    swap = 0
    orientation = [orientation, -orientation]
    new_orientation = np.array(orientation)


# Creating the car class

class Car(Widget):
    
    angle = NumericProperty(0)
    rotation = float(0)
    velocity_x = NumericProperty(0)
    velocity_y = NumericProperty(0)
    velocity = ReferenceListProperty(velocity_x, velocity_y)


    def move(self, rotation):
        self.pos = Vector(*self.velocity) + self.pos
        self.rotation = rotation
        self.angle = self.angle + self.rotation
        # print('angle in move is ', self.angle)
        # print('rotation in move is ', self.rotation)

        xx = goal_x - self.x
        yy = goal_y - self.y
        orientation = Vector(*self.velocity).angle((xx,yy)) #SOMA added /180.
        # print('orientation in move is ', orientation)

        area = (self.x - 80, self.y - 80, self.x + 80, self.y + 80)

        img = PILImage.open("./images/mask.png").convert('L')
        temp = img.rotate(90, expand=True).crop(area)
        cropped_image = temp.rotate(orientation, expand=True) #SOMA - removed expand
        #print(cropped_image.size)
        #cropped_image.save('./images/croppedimage.png')

        #im = PILImage.open('./images/croppedimage.png')
        width, height = cropped_image.size   # Get dimensions
        #print(cropped_image.size)

        left = (width - 80)/2
        top = (height - 80)/2
        right = (width + 80)/2
        bottom = (height + 80)/2

        # Crop the center of the image
        im = cropped_image.crop((left, top, right, bottom))
        
        #im.save('./images/final.jpg')
        im.thumbnail((32,32))

        return im


class Game(Widget):

    car = ObjectProperty(None)

    reward_window = []

    start_timesteps = 4000 #1e4 # 3000 #Number of iterations/timesteps before which the model randomly chooses an action, and after which it starts to use the policy network
    #eval_freq = 5 # How often the evaluation step is performed (after how many timesteps)
    max_timesteps = 12000 #5e5 # 5000 #Total number of iterations/timesteps
    max_episodes = 20  # max num of episodes
    eval_episode = 1
    
    expl_noise = 0.1 # Exploration noise - STD value of exploration Gaussian noise
    batch_size = 2 # Size of the batch
    discount = 0.99 # Discount factor gamma, used in the calculation of the total discounted reward
    tau = 0.005 # Target network update rate
    policy_noise = 0.2 # STD of Gaussian noise added to the actions for the exploration purposes
    noise_clip = 0.5 # Maximum value of the Gaussian noise added to the actions (policy)
    policy_freq = 2 # Number of iterations to wait before the policy network (Actor model) is updated

    total_timesteps = 0
    timesteps_since_eval = 0
    episode_timesteps = 0
    done = True
    running_score = 0
    done_counter = 0

    """ parameters for epsilon decay """
    epsilon_start = 1
    epsilon_final = 0.01
    decay_rate = max_episodes / 50


    def serve_car(self):
        self.car.center = self.center
        self.car.velocity = Vector(6, 0)


    def get_orientation(self):
        xx = goal_x - self.car.x
        yy = goal_y - self.car.y
        orientation = Vector(*self.car.velocity).angle((xx,yy))/180. #SOMA - added /180.
        #print('orientation is ', orientation)
        return orientation


    def reset(self):
        self.car.x = np.random.randint(50, 1400)
        self.car.y = np.random.randint(50, 600)

        # self.car.x = 800 #goal_x
        # self.car.y = 300 #goal_y
        # self.car.velocity = Vector(6, 0)

        xx = goal_x - self.x
        yy = goal_y - self.y
        orientation = Vector(*self.car.velocity).angle((xx,yy))

        area = (self.car.x - 80, self.car.y - 80, self.car.x + 80, self.car.y + 80)

        img = PILImage.open("./images/mask.png").convert('L')
        temp = img.rotate(90, expand=True).crop(area)
        cropped_image = temp.rotate(orientation, expand=True) #SOMA - removed expand
        #print(cropped_image.size)
        #cropped_image.save('./images/croppedimagereset.png')

        #im = PILImage.open('./images/croppedimage.png')
        width, height = cropped_image.size   # Get dimensions

        left = (width - 80)/2
        top = (height - 80)/2
        right = (width + 80)/2
        bottom = (height + 80)/2

        # Crop the center of the image
        im = cropped_image.crop((left, top, right, bottom))
        
        #im.save('./images/finalreset.jpg')

        im.thumbnail((32,32))

        self.stack = [np.expand_dims(im, axis=0)]  #SOMA COMMENTED OFF
        # print(self.stack)

        return torch.FloatTensor(self.stack).permute(1, 0, 2, 3) #SOMA COMMENTED OFF

    
    def evaluate(self):
        brain.load(file_name, './pytorch_models/')


    def eval_reset(self):
        xx = goal_x - self.x
        yy = goal_y - self.y
        orientation = Vector(*self.car.velocity).angle((xx,yy))

        area = (self.car.x - 80, self.car.y - 80, self.car.x + 80, self.car.y + 80)

        img = PILImage.open("./images/mask.png").convert('L')
        temp = img.rotate(90, expand=True).crop(area)
        cropped_image = temp.rotate(orientation, expand=True) #SOMA - removed expand
        #print(cropped_image.size)
        #cropped_image.save('./images/croppedimagereset.png')

        #im = PILImage.open('./images/croppedimage.png')
        width, height = cropped_image.size   # Get dimensions

        left = (width - 80)/2
        top = (height - 80)/2
        right = (width + 80)/2
        bottom = (height + 80)/2

        # Crop the center of the image
        im = cropped_image.crop((left, top, right, bottom))
        
        #im.save('./images/finalreset.jpg')

        im.thumbnail((32,32))

        self.stack = [np.expand_dims(im, axis=0)]

        return torch.FloatTensor(self.stack).permute(1, 0, 2, 3)

    def eval_update(self, dt):
        global brain
        global reward
        global scores
        global distance
        global orientation
        global goal_x
        global goal_y
        global longueur
        global largeur
        global swap

        global _max_episode_steps
        global _max_eval_episodes
        global episode_num
        global episode_reward
        global state
        goal_x = 1000
        goal_y = 400
        #episode_reward = 0
        #episode_num = 0

        longueur = self.width
        largeur = self.height
        if first_update:
            init()

        if self.total_timesteps < self.max_timesteps:
            if self.done:
                if self.total_timesteps != 0:
                    print("Total Timesteps: {} Episode Num: {} Reward: {}".format(self.total_timesteps, episode_num, episode_reward))

                state = self.eval_reset()
                #print('AFTER TRAIN')

                # Set the Done to False
                self.done = False

                # Set rewards and episode timesteps to zero
                episode_reward = 0
                self.episode_timesteps = 0
                episode_num += 1
                self.eval_episode += 1

            # Before 10000 timesteps, we play random actions
            # if self.total_timesteps < self.start_timesteps:
            #     action = np.random.uniform(-max_action, max_action, (1,))
            # else:
            orientation = self.get_orientation()
            orientation = [orientation, -orientation]
            orientation = np.array(orientation)
            action = brain.select_action(state, orientation)
            print('action from select_action is ', action)
            # If the explore_noise parameter is not 0, we add noise to the action and we clip it
            if self.expl_noise != 0:
                expl_noise = (self.epsilon_start - self.epsilon_final) * math.exp(-1. * self.total_timesteps / self.decay_rate)
                action = (action + np.random.normal(0, expl_noise, size=action_dim)).clip(-max_action, max_action)

            next_state, new_orientation, reward, self.done = self.step(action) #new_distance,

            scores.append(brain.score())

            # We check if the episode is done
            done_bool = 1 if self.episode_timesteps + 1 == _max_episode_steps else float(self.done)
            if self.episode_timesteps + 1 == _max_episode_steps:
                self.done = True
  
            # We increase the total reward
            episode_reward += reward
  
            # We store the new transition into the Experience Replay memory (ReplayBuffer)
            replay_buffer.add(state, next_state, orientation, new_orientation, action, reward, done_bool) #distance, new_distance, 

            # We update the state, the episode timestep, the total timesteps, and the timesteps since the evaluation of the policy
            state = next_state
            orientation = new_orientation
            self.episode_timesteps += 1
            self.total_timesteps += 1
            self.timesteps_since_eval += 1


    def step(self, action):
        global goal_x
        global goal_y
        global distance
        global swap
        global sand_penalty
        global distance_penalty
        global boundary_penalty
        global goal_reward
        global goal_penalty

        cropped_image = self.car.move(action.tolist()[0])

        self.stack.pop(0)
        self.stack.append(np.expand_dims(cropped_image, axis=0))

        next_state = torch.FloatTensor(self.stack).permute(1, 0, 2, 3)

        new_distance = np.sqrt((self.car.x - goal_x)**2 + (self.car.y - goal_y)**2)

        new_orientation = self.get_orientation()
        new_orientation = [new_orientation, -new_orientation]
        new_orientation = np.array(new_orientation)

        if sand[int(self.car.x),int(self.car.y)] > 0:
            self.car.velocity = Vector(0.5, 0).rotate(self.car.angle)
            #print(1, goal_x, goal_y, new_distance, int(self.car.x),int(self.car.y), im.read_pixel(int(self.car.x),int(self.car.y)))
            reward = -0.5
            sand_penalty += 0.5
            self.done = False
        else: # otherwise
            self.car.velocity = Vector(2, 0).rotate(self.car.angle)
            reward = 10
            self.done = False
            #print(0, goal_x, goal_y, new_distance, int(self.car.x),int(self.car.y), im.read_pixel(int(self.car.x),int(self.car.y)))
            if new_distance < distance:
                reward = 20
                distance_penalty += 20
                #self.done = False
            # else:
            #     reward = -10

        if self.car.x < 5:
            self.car.x = 5
            reward -= 10
            boundary_penalty += 10
            self.done = True
        if self.car.x > self.width - 5:
            self.car.x = self.width - 5
            reward -= 10
            boundary_penalty += 10
            self.done = True
        if self.car.y < 5:
            self.car.y = 5
            reward -= 10
            boundary_penalty += 10
            self.done = True
        if self.car.y > self.height - 5:
            self.car.y = self.height - 5
            reward -= 10
            boundary_penalty += 10
            self.done = True

        # if self.car.x < 20:
        #     self.car.x = 20
        #     reward -= 50
        #     self.done = True
        # if self.car.x > self.width - 20:
        #     self.car.x = self.width - 20
        #     reward -= 50
        #     self.done = True
        # if self.car.y < 20:
        #     self.car.y = 20
        #     reward -= 50
        #     self.done = True
        # if self.car.y > self.height - 20:
        #     self.car.y = self.height - 20
        #     reward -= 50
        #     self.done = True


        if new_distance < 25:
            reward += 35
            goal_reward += 35
            if swap == 1:
                with open("./logs/modifiedRewards.txt", 'a') as f:
                    sys.stdout = f
                    print("Reached the target: x: %s, y: %s", self.car.x, self.car.y)
                goal_x = 1000
                goal_y = 400
                swap = 0
                self.done = False #SOMA - Changed to False
            else:
                with open("./logs/modifiedRewards.txt", 'a') as f:
                    sys.stdout = f
                    print("Reached the target: x: %s, y: %s", self.car.x, self.car.y) 
                goal_x = 200
                goal_y = 100
                swap = 1
                self.done = True
        else:
            reward -= 0.1
            goal_penalty += 0.1
        distance = new_distance

        return next_state, new_orientation, reward, self.done, sand_penalty, distance_penalty, boundary_penalty, goal_reward, goal_penalty #new_distance, 


    def update(self, dt):

        global brain
        global reward
        global scores
        global last_distance
        global orientation
        global goal_x
        global goal_y
        global longueur
        global largeur
        global swap

        global _max_episode_steps
        global episode_num
        global episode_reward
        global state
        #episode_reward = 0
        #episode_num = 0

        global sand_penalty
        global distance_penalty
        global boundary_penalty
        global goal_reward
        global goal_penalty

        longueur = self.width
        largeur = self.height
        if first_update:
            init()

        if self.total_timesteps < self.max_timesteps:

            if self.done:
                self.done_counter += 1
                if self.total_timesteps != 0:
                    #print("Total Timesteps: {} Episode Num: {} Reward: {}".format(self.total_timesteps, episode_num, episode_reward))
                    with open("./logs/modifiedRewards.txt", 'a') as f:
                        sys.stdout = f
                        print("Total Steps: ", self.total_timesteps, "Episode: ",episode_num, "Reward: ", episode_reward,"Episode TimeSteps: ", self.episode_timesteps, "Sand Penalty: ", sand_penalty, "Distance Penalty: ", distance_penalty, "Boundary Penalty: ", boundary_penalty, "Goal Reward: ", goal_reward, "Goal Penalty: ", goal_penalty)
                    brain.train(replay_buffer, self.episode_timesteps)

                state = self.reset()
                #print('AFTER TRAIN')

                # Set the Done to False
                self.done = False

                # Set rewards and episode timesteps to zero
                episode_reward = 0
                self.episode_timesteps = 0
                episode_num += 1
                sand_penalty = 0
                distance_penalty = 0
                boundary_penalty = 0
                goal_reward = 0
                goal_penalty = 0

            # Before 10000 timesteps, we play random actions
            if self.total_timesteps < self.start_timesteps:
                action = np.random.uniform(-max_action, max_action, (1,))
                # with open("./logs/modifiedRewards.txt", 'a') as f:
                #     sys.stdout = f
                #     print('random action is ', action)
            else:
                orientation = self.get_orientation()
                orientation = [orientation, -orientation]
                orientation = np.array(orientation)
                action = brain.select_action(state, orientation)
                #logging.info('Action from select_action at %s total_timestep is %s'%(self.total_timesteps, action))
                #print('action from select_action is ', action)
                # If the explore_noise parameter is not 0, we add noise to the action and we clip it
                if self.expl_noise != 0:
                    expl_noise = (self.epsilon_start - self.epsilon_final) * math.exp(-1. * self.total_timesteps / self.decay_rate)
                    action = (action + np.random.normal(0, self.expl_noise, size=action_dim)).clip(-max_action, max_action)

            next_state, new_orientation, reward, self.done, sand_penalty, distance_penalty, boundary_penalty, goal_reward, goal_penalty = self.step(action) #new_distance,  

            scores.append(brain.score())

            # We check if the episode is done
            done_bool = 1 if self.episode_timesteps + 1 == _max_episode_steps else float(self.done)
            if self.episode_timesteps + 1 == _max_episode_steps:
                self.done = True

            if self.done and self.total_timesteps > self.start_timesteps:
                if episode_reward > 0: #and self.done_counter % 3 == 0:
                    if save_models: brain.save("%s_%s_%s" % (file_name, int(episode_reward), self.done_counter), directory="./pytorch_models")
  
            # We increase the total reward
            episode_reward += reward


            # print('BEFORE ADDING TO REPLAY BUFFER')
            # print('shape of state is ', type(state))
            # print('type of orientation is ', type(orientation))
            # print('type of reward is ', type(reward))
            # print('orientation is ', orientation)
            # We store the new transition into the Experience Replay memory (ReplayBuffer)
            replay_buffer.add(state, next_state, orientation, new_orientation, action, reward, done_bool)

            # We update the state, the episode timestep, the total timesteps, and the timesteps since the evaluation of the policy
            state = next_state
            orientation = new_orientation
            self.episode_timesteps += 1
            self.total_timesteps += 1

# Adding the painting tools

class MyPaintWidget(Widget):

    def on_touch_down(self, touch):
        global length, n_points, last_x, last_y
        with self.canvas:
            Color(0.8,0.7,0)
            d = 10.
            touch.ud['line'] = Line(points = (touch.x, touch.y), width = 10)
            last_x = int(touch.x)
            last_y = int(touch.y)
            n_points = 0
            length = 0
            sand[int(touch.x),int(touch.y)] = 1
            img = PILImage.fromarray(sand.astype("uint8")*255)
            img.save("./images/sand.jpg")

    def on_touch_move(self, touch):
        global length, n_points, last_x, last_y
        if touch.button == 'left':
            touch.ud['line'].points += [touch.x, touch.y]
            x = int(touch.x)
            y = int(touch.y)
            length += np.sqrt(max((x - last_x)**2 + (y - last_y)**2, 2))
            n_points += 1.
            density = n_points/(length)
            touch.ud['line'].width = int(20 * density + 1)
            sand[int(touch.x) - 10 : int(touch.x) + 10, int(touch.y) - 10 : int(touch.y) + 10] = 1

            
            last_x = x
            last_y = y

# Adding the API Buttons (clear, save and load)

class CarApp(App):

    def build(self):
        parent = Game()
        parent.serve_car()
        #For Training
        Clock.schedule_interval(parent.update, 1.0/60.0)
        #For Inferencing
        #parent.evaluate()
        #Clock.schedule_interval(parent.eval_update, 1.0/60.0)
        self.painter = MyPaintWidget()
        clearbtn = Button(text = 'clear')
        savebtn = Button(text = 'save', pos = (parent.width, 0))
        loadbtn = Button(text = 'load', pos = (2 * parent.width, 0))
        clearbtn.bind(on_release = self.clear_canvas)
        savebtn.bind(on_release = self.save)
        loadbtn.bind(on_release = self.load)
        parent.add_widget(self.painter)
        parent.add_widget(clearbtn)
        parent.add_widget(savebtn)
        parent.add_widget(loadbtn)
        return parent

    def clear_canvas(self, obj):
        global sand
        self.painter.canvas.clear()
        sand = np.zeros((longueur,largeur))

    def save(self, obj):
        print("saving brain...")
        brain.save()
        plt.plot(scores)
        plt.show()

    def load(self, obj):
        print("loading last saved brain...")
        brain.load()

# Running the whole thing
if __name__ == '__main__':
    CarApp().run()