...
class Game(Widget):
    ...
    start_timesteps = 160000 #1e4 # 3000 #Number of iterations/timesteps before which the model randomly chooses an action, and after which it starts to use the policy network
    #eval_freq = 5 # How often the evaluation step is performed (after how many timesteps)
    max_timesteps = 480000 #5e5 # 5000 #Total number of iterations/timesteps
    max_episodes = 800  # max num of episodes
    eval_episode = 1
        ...
        if self.total_timesteps < self.max_timesteps:
            if self.done:
                if self.total_timesteps != 0:
                    ...
                    with open("./logs/eval_final.txt", 'a') as f:
                        sys.stdout = f
    ...
    def step(self, action):
        ...
        if new_distance < 25:
            ...
            goal_reward += 35
            if swap == 1:
                with open("./logs/final.txt", 'a') as f:
                    sys.stdout = f
