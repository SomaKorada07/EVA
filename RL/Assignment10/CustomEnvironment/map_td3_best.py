...
class ReplayBuffer(object):
  ...
  def add(self, state, new_state, distance, new_distance, action, reward, done_bool):
    data = (state, new_state, distance, new_distance, action, reward, done_bool)
    #data = (state, new_state, new_distance, action, reward, done_bool)
  ...
  def sample(self, batch_size):
    ...
    batch_states, batch_next_states, batch_distances, batch_next_distances, batch_actions, batch_rewards, batch_dones = [], [], [], [], [], [], []
    #batch_states, batch_next_states, batch_next_distances, batch_actions, batch_rewards, batch_dones = [], [], [], [], [], []
    for i in ind:
      state, next_state, distance, next_distance, action, reward, done = self.storage[i]
    ...
    return np.array(batch_states), np.array(batch_next_states), np.array(batch_distances).reshape(-1, 1), np.array(batch_next_distances).reshape(-1, 1), np.array(batch_actions), np.array(batch_rewards).reshape(-1, 1), np.array(batch_dones).reshape(-1, 1)
    #return np.array(batch_states), np.array(batch_next_states), np.array(batch_next_distances).reshape(-1, 1), np.array(batch_actions), np.array(batch_rewards).reshape(-1, 1), np.array(batch_dones).reshape(-1, 1)
...
class Car(Widget):
    ...
    def move(self, rotation):
        ...
        car_image = PILImage.open("./images/car.png").resize((20,10))
        ...
        # x_coord1 = int(self.x)-80 if self.x > 80 else 0
        # x_coord2 = int(self.x)+80 if self.x < 1340 else 1420
        # y_coord1 = int(self.y)-80 if self.y > 80 else 0
        # y_coord2 = int(self.y)+80 if self.y < 542 else 622
...
class Game(Widget):
    ...
    start_timesteps = 6000 #15 #1e4 # Number of iterations/timesteps before which the model randomly chooses an action, and after which it starts to use the policy network
    ...
    def update(self, dt):
        ...
        if self.total_timesteps < self.max_timesteps:
            ...
            if self.done: #or self.total_timesteps == (self.max_timesteps - 1):
            ...
            if self.total_timesteps < self.start_timesteps:
                ...
                action = brain.select_action(state, torch.FloatTensor(np.array(distance)))
            ...
            done_bool = 1 if self.episode_timesteps + 1 == _max_episode_steps else float(self.done) #SOMA - made 1000 to 50
            ...
            replay_buffer.add(state, next_state, distance, new_distance, action, reward, done_bool)
